...
async function inspect() {
  try {
    ...
    const propRes = await)¢Ïol.query('SELECT property_id, title FROM properties WHERE owner_id = $1', [user.user_id]);
    const propertyIds = propRes.rows.map(r => r.property_id);
    console.log('Properties:*, propRes.rows.map(r => r.property_id));
