    ...
    <div className="w-full bg-[#FAF8F3] min-h-screen pb-16 font-sans">
      <div className=" px-4 sm:px-8 lg:px-12 xl:px-4">
        ...
        <div className="flex items-center text-sm font-semibold text-gray-500 gap-2 mb-8">
          ...
          <span className="text-[#1a2b25]">Inspection Reports</span>
        </div>

        <div className="mb-10">
          <h1 className="text-3xl sm:text-4xl font-serif font-bold text-[#1a2b25] mb-2">Inspection Reports</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
          ...
          <button 
            ...
            className={`text-left rounded-[20px] shadow-sm p-6 flex items-center gap-4 transition-all focus:outline-none focus:ring-2 focus:ring-[#B8860B] focus:ring-offset-2 ${
          ...
          >
            <div className={`w-14 h-14 rounded-full flex items-center justify-center shrink-0 transition-colors ${
              activeSummaryFilter === 'all' ? 'bg-[#1E5631] text-white' : 'bg-[#eaf1ec] text-[#1E5631]'
            }`}>
              <FileSearch size={24} />
            </div>
            <div>
          ...
          <button 
            ...
            className={`text-left rounded-[20px] shadow-sm p-6 flex items-center gap-4 transition-all focus:outline-none focus:ring-2 focus:ring-[#B8860B] focus:ring-offset-2 ${
              activeSummaryFilter === 'properties' 
                ? 'bg-white border-2 border-[#1E5631] shadow-md ring-1 ring-[#1E5631]/10' 
                : 'bg-white border border-gray-100 hover:border-gray-300'
            }`}
          >
            <div className={`w-14 h-14 rounded-full flex items-center justify-center shrink-0 transition-colors ${
              activeSummaryFilter === 'properties' ? 'bg-[#B8860B] text-white' : 'bg-[#f4ebd0] text-[#B8860B]'
            }`}>
              <MapPin size={24} />
            </div>
            <div>
          ...
          <button 
            ...
            className={`text-left rounded-[20px] shadow-sm p-6 flex items-center gap-4 transition-all focus:outline-none focus:ring-2 focus:ring-[#B8860B] focus:ring-offset-2 ${
              activeSummaryFilter === 'latest' 
                ? 'bg-white border-2 border-[#1E5631] shadow-md ring-1 ring-[#1E5631]/10' 
                : 'bg-white border border-gray-100 hover:border-gray-300'
            }`}
          >
            <div className={`w-14 h-14 rounded-full flex items-center justify-center shrink-0 transition-colors ${
              activeSummaryFilter === 'latest' ? 'bg-gray-800 text-white' : 'bg-gray-50 text-gray-500'
            }`}>
              <Calendar size={24} />
            </div>
            <div>
