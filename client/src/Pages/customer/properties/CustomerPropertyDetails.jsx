...
const CustomerPropertyDetails = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#FAF8F3] flex flex-col items-center justify-center">
        <h2 className="text-xl font-bold text-gray-800 animate-pulse">{t('propertyDetails:loadingDetails')}</h2>
      </div>
    );
  }
  ...
  if (error || !propertyData) {
    return (
      <div className="min-h-screen bg-[#FAF8F3] flex flex-col items-center justify-center">
        <h2 className="text-2xl font-bold text-red-800 mb-2">{error || t('propertyDetails:notFound')}</h2>
    ...
    <div className="w-full bg-[#FAF8F3] min-h-screen pb-16 font-sans">
      ...
      <div className="pt-6 px-4 sm:px-8 lg:px-12 xl:px-14">
        <div className="flex items-center text-sm font-semibold text-gray-500 gap-2 mb-6">
          ...
          <ChevronRight size={14} className="text-gray-400 rtl:rotate-180" />
          <span className="text-[#1a2b25]">{t('propertyDetails:propertyDetails')}</span>
        </div>
      </div>
      ...
      <div className="px-4 sm:px-8 lg:px-12 xl:px-14 max-w-[1200px] mx-auto space-y-8">
          ...
          <div className="w-full h-[350px] md:h-[500px] relative bg-[#FAF8F3] flex items-center justify-center group overflow-hidden">
            {allImages.length > 0 ? (
              <>
                ...
                <div className="absolute top-4 end-4 bg-black/50 backdrop-blur-md text-white text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-sm z-10">
                ...
                {allImages.length > 1 && (
                  <>
                    <button
                      ...
                      className="absolute start-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/80 hover:bg-white text-[#1a2b25] flex items-center justify-center shadow-[0_2px_10px_rgba(0,0,0,0.1)] opacity-100 md:opacity-0 group-hover:opacity-100 transition-all duration-200 z-10 rtl:rotate-180"
                    >
                      <ChevronLeft size={24} className="mr-0.5" />
                    </button>
                    <button
                      onClick={handleNextImage}
                      className="absolute end-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/80 hover:bg-white text-[#1a2b25] flex items-center justify-center shadow-[0_2px_10px_rgba(0,0,0,0.1)] opacity-100 md:opacity-0 group-hover:opacity-100 transition-all duration-200 z-10 rtl:rotate-180"
                    >
              ...
              <div className="flex flex-col items-center justify-center text-gray-400">
                ...
                <span className="text-sm font-bold">{t('propertyDetails:noImages')}</span>
              </div>
            )}
          ...
          <div className="p-6 sm:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white">
            <div className="flex-1">
              ...
              <div className="text-gray-600 font-medium text-sm sm:text-base mb-4 flex items-center">
                <MapPin size={16} className="me-2 text-[#B8860B]" />
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm font-semibold text-[#2c4232]">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-gray-400 font-normal">{t('propertyDetails:use')}</span> {property.property_use || '--'}
                </div>
                <div className="w-1 h-1 rounded-full bg-gray-300"></div>
                <div className="flex items-center gap-1.5">
            ...
            <div className="w-full md:w-auto flex-shrink-0">
              ...
              >
                <Edit2 size={16} /> {t('propertyDetails:editProperty')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8">
          ...
          <div className="bg-white rounded-[24px] shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border border-gray-100/60 p-8 sm:p-10">
            <SectionTitle icon={Map} title={t('propertyDetails:locationInfo')} />
            <div className="grid grid-cols-2 md:grid-cols-4 gap-y-8 gap-x-6">
              <DetailItem label={t('propertyDetails:country')} value={location?.country_english} />
              <DetailItem label={t('propertyDetails:province')} value={location?.province_english} />
              <DetailItem label={t('propertyDetails:division')} value={location?.division_english} />
              <DetailItem label={t('propertyDetails:district')} value={location?.district_english} />
              <DetailItem label={t('propertyDetails:tehsil')} value={location?.tehsil_english} />
              <DetailItem label={t('propertyDetails:city')} value={location?.city_english} />
              <DetailItem label={t('propertyDetails:society')} value={location?.society_english} />
              <DetailItem label={t('propertyDetails:areaBlock')} value={location?.area_english} />
              <DetailItem label={t('propertyDetails:locationType')} value={property.property_location} />
            </div>
          </div>

          <div className="bg-white rounded-[24px] shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border border-gray-100/60 p-8 sm:p-10">
            <SectionTitle icon={Ruler} title={t('propertyDetails:sizeArea')} />

            <h4 className="text-sm font-bold text-[#1a2b25] mb-5">{t('propertyDetails:primarySize')}</h4>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-y-6 gap-x-6 mb-8">
              <DetailItem label={t('propertyDetails:propertySize')} value={property.property_size} />
              <DetailItem label={t('propertyDetails:sizeUom')} value={property.uom_english} />
              <DetailItem label={t('propertyDetails:marlaSizeRef')} value={property.marla_size_sqft ? `${property.marla_size_sqft} SqFt` : '--'} />
            </div>

            <h4 className="text-sm font-bold text-[#1a2b25] mb-5 border-t border-gray-100 pt-6">{t('propertyDetails:calculatedAreas')}</h4>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-y-6 gap-x-6 mb-8">
              <DetailItem label={t('propertyDetails:areaMarla')} value={property.property_area_marla} />
              <DetailItem label={t('propertyDetails:areaKanal')} value={property.property_area_kanal} />
              <DetailItem label={t('propertyDetails:areaAcre')} value={property.property_area_acre} />
              <DetailItem label={t('propertyDetails:areaSqFt')} value={property.property_area_sqft} />
              <DetailItem label={t('propertyDetails:areaSqYard')} value={property.property_area_sqyard} />
            </div>

            <h4 className="text-sm font-bold text-[#1a2b25] mb-5 border-t border-gray-100 pt-6">{t('propertyDetails:constructionArea')}</h4>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-6">
              <DetailItem label={t('propertyDetails:coveredAreaSqFt')} value={property.property_covered_area_sqft} />
              <DetailItem label={t('propertyDetails:openAreaSqFt')} value={property.property_open_area_sqft} />
              <DetailItem label={t('propertyDetails:dimensionFront')} value={property.property_size_front} />
              <DetailItem label={t('propertyDetails:dimensionBack')} value={property.property_size_back} />
            </div>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-white rounded-[24px] shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border border-gray-100/60 p-8 sm:p-10">
              <SectionTitle icon={List} title={t('propertyDetails:propertyParticulars')} />
              <div className="grid grid-cols-2 md:grid-cols-3 gap-y-8 gap-x-6">
                <DetailItem label={t('propertyDetails:rooms')} value={property.property_rooms} />
                <DetailItem label={t('propertyDetails:bathrooms')} value={property.property_bath_rooms} />
                <DetailItem label={t('propertyDetails:floors')} value={property.property_floors} />
                <DetailItem label={t('propertyDetails:lounges')} value={property.property_lounges} />
                <DetailItem label={t('propertyDetails:kitchens')} value={property.property_kitchens} />
                <DetailItem label={t('propertyDetails:drawingRooms')} value={property.property_drawing_rooms} />
              </div>
            </div>

            <div className="bg-white rounded-[24px] shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border border-gray-100/60 p-8 sm:p-10">
              <SectionTitle icon={Navigation} title={t('propertyDetails:roadAccess')} />
              <div className="grid grid-cols-2 gap-y-8 gap-x-6">
                <DetailItem label={t('propertyDetails:frontRoad')} value={property.property_road_size_front_ft} />
                <DetailItem label={t('propertyDetails:backRoad')} value={property.property_road_size_back_ft} />
                <DetailItem label={t('propertyDetails:leftRoad')} value={property.property_road_size_left_ft} />
                <DetailItem label={t('propertyDetails:rightRoad')} value={property.property_road_size_right_ft} />
              </div>
            </div>
          </div>

          <div className="bg-white rounded-[24px] shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border border-gray-100/60 p-8 sm:p-10">
            <SectionTitle icon={CheckSquare} title={t('propertyDetails:featuresAmenities')} />

            <div className="grid grid-cols-2 md:grid-cols-5 gap-y-8 gap-x-6 mb-10">
              <DetailItem label={t('propertyDetails:swimmingPool')} value={property.property_swimming_pool ? t('propertyDetails:yes') : t('propertyDetails:no')} />
              <DetailItem label={t('propertyDetails:mediaRoom')} value={property.property_media_room ? t('propertyDetails:yes') : t('propertyDetails:no')} />
              <DetailItem label={t('propertyDetails:solarInstalled')} value={property.property_solar_is_installed ? `${t('propertyDetails:yes')} (${property.property_solar_capacity || t('propertyDetails:unknownCapacity')})` : t('propertyDetails:no')} />
              <DetailItem label={t('propertyDetails:electricMeters')} value={property.property_electric_meters} />
              <DetailItem label={t('propertyDetails:gasMeters')} value={property.property_gas_meters} />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4">{t('propertyDetails:amenities')}</h4>
                <div className="flex flex-wrap gap-3">
                  {amenities.length > 0 ? (
                    ...
                    <span className="text-sm font-semibold text-gray-400">{t('propertyDetails:noAmenities')}</span>
                  )}
                </div>
              </div>

              <div>
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4">{t('propertyDetails:description')}</h4>
                <div className="bg-[#f4f2ea] p-4 rounded-xl text-sm text-[#1a2b25]">
                  {property.property_description || <span className="text-gray-400 font-semibold">{t('propertyDetails:noDescription')}</span>}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[24px] shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border border-gray-100/60 p-8 sm:p-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div>
                <SectionTitle icon={ImageIcon} title={t('propertyDetails:pictures')} />
                {pictures.length > 0 ? (
                  ...
                  <div className="h-40 rounded-2xl border-2 border-dashed border-gray-200 flex items-center justify-center text-sm font-bold text-gray-400 bg-[#FAF8F3]">
                    {t('propertyDetails:noPictures')}
                  </div>
                )}
              </div>
              <div>
                <SectionTitle icon={Video} title={t('propertyDetails:videos')} />
                {video ? (
          ...
          <div id="pricing-demand-section" className="bg-white rounded-[24px] shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border border-[#e4d7be] overflow-hidden">
            <div className="bg-[#FAF8F3] px-8 py-6 border-b border-[#e4d7be]">
              <h3 className="text-xl font-serif font-bold text-[#1a2b25] flex items-center gap-2">
                ...
                {t('propertyDetails:pricingDemand')}
              </h3>
            </div>
            {demandPageError && (
            ...
            <div className="p-8 flex items-center justify-center">
              {demand ? (
                <div className="max-w-md w-full bg-[#FAF8F3] rounded-3xl p-8 border border-[#e4d7be]/60 relative overflow-hidden">
                  <div className="absolute top-0 end-0 w-40 h-40 rounded-full -me-12 -mt-12 bg-[#B8860B]/10" />
                  <div className="relative">
                    <div className="inline-block px-3 py-1 bg-white border border-gray-200 rounded-full text-[11px] font-bold text-gray-600 uppercase tracking-wider mb-6">
                      {t('propertyDetails:for')} {demand.demand_type}
                    </div>
                    <div className="mb-4">
                      <p className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-2">{t('propertyDetails:currentDemand')}</p>
                      ...
                      <p className="text-sm font-semibold text-gray-500 flex items-center gap-1.5">
                        <Calendar size={14} />
                        {t('propertyDetails:effectiveFrom')} {new Date(demand.effective_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
                      </p>
                    </div>
                    <button
                    ...
                    >
                      {t('propertyDetails:changeDemand')}
                    </button>
                  </div>
                </div>
              ) : (
                <div className="w-full">
                  <div className="flex flex-col items-center justify-center text-gray-400 py-6 mb-4">
                    ...
                    <span className="text-lg font-bold text-[#1a2b25]">{t('propertyDetails:pricingNotSet')}</span>
                    <p className="text-sm font-medium mt-2">{t('propertyDetails:pricingOfferPrompt')}</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto pb-4">
                    ...
                    >
                      ...
                      <span className="text-lg font-bold text-[#1a2b25]">{t('propertyDetails:forSale')}</span>
                    </button>
                    ...
                    >
                      ...
                      <span className="text-lg font-bold text-[#1a2b25]">{t('propertyDetails:forRent')}</span>
                    </button>
                  </div>
                </div>
              )}
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-fadeIn">
            ...
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-[#FAF8F3]">
              <h3 className="text-lg font-bold text-[#1a2b25]">
                {demand ? t('propertyDetails:changeTypeDemand', { type: selectedDemandType.demand_type_english }) : t('propertyDetails:setTypeDemand', { type: selectedDemandType.demand_type_english })}
              </h3>
            ...
            <form onSubmit={handleSubmitDemand} className="p-6">
              ...
              {demand && demand.demand_type === selectedDemandType.demand_type_english && (
                <div className="mb-6 bg-gray-50 p-4 rounded-2xl border border-gray-100">
                  <span className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">{t('propertyDetails:currentAmount')}</span>
              ...
              <div className="space-y-5">
                <div>
                  <label className="block text-sm font-bold text-[#1a2b25] mb-2">{t('propertyDetails:newDemandAmount')} <span className="text-red-500">*</span></label>
                  <input
                    type="number"
                ...
                >
                  {submitLoading ? (
                    <>
                      ...
                      {t('propertyDetails:saving')}
                    </>
                  ) : (
                    t('propertyDetails:saveNewDemand')
                  )}
                </button>
              </div>
