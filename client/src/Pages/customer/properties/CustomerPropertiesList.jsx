        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6 w-full sm:w-auto shrink-0">
            <span 
              ...
              dangerouslySetInnerHTML={{ __html: `${filteredProperties.length} ${t('properties:propertiesFound')}` }}
            />
            <button className="flex items-center justify-center gap-2 px-6 py-3.5 border border-gray-200 bg-white rounded-2xl text-sm font-bold text-gray-800 hover:bg-gray-50 transition-colors shadow-sm w-full sm:w-auto">
              <Filter size={16} /> {t('properties:filters')}
            </button>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {loading ? (
            <div className="py-20 flex flex-col items-center justify-center text-center bg-white rounded-[24px] border border-gray-100 shadow-sm">
              ...
              <p className="text-sm font-medium text-gray-500">{t('properties:loadingProperties')}</p>
            </div>
          ) : error ? (
            <div className="py-20 flex flex-col items-center justify-center text-center bg-white rounded-[24px] border border-gray-100 shadow-sm">
              <p className="text-sm font-medium text-red-500 mb-4">{error}</p>
              <button onClick={() => window.location.reload()} className="px-4 py-2 bg-gray-100 rounded-full text-xs font-bold text-gray-700 hover:bg-gray-200">{t('common:retry')}</button>
            </div>
          ) : (
              ...
              >
                ...
                <div className="h-[240px] xl:w-[360px] shrink-0 relative rounded-[16px] overflow-hidden bg-[#FAF8F3] flex items-center justify-center">
                  ...
                  {property.total_photos > 0 && (
                    <div className="absolute bottom-3 end-3 bg-black/70 backdrop-blur-sm text-white text-[11px] font-semibold px-2.5 py-1 rounded-md flex items-center gap-1.5">
                      <ImageIcon size={12} /> {property.total_photos} {t('properties:photos')}
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col flex-1 py-1 xl:flex-row xl:justify-between">
                      ...
                      {property.rooms > 0 && (
                        <>
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {property.rooms} {property.property_type === 'Commercial' ? t('properties:rooms') : t('properties:bedrooms')}
                          </div>
                        </>
                      )}
                      ...
                      {property.floors > 0 && (
                        <>
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {property.floors} {t('properties:floors')}
                          </div>
                        </>
                      )}
                    </div>
                  ...
                  <div className="flex flex-col justify-between items-end shrink-0 w-full xl:w-[160px] mt-6 xl:mt-0">
                    ...
                    <div className="flex xl:flex-col gap-3 w-full mt-auto">
                      ...
                      >
                        {t('properties:viewDetails')} <ChevronRight size={14} className="rtl:rotate-180" />
                      </button>
                      <button 
                        onClick={(e) => {
                          e.stopPropagation();
                      ...
                      >
                        <Edit2 size={14} /> {t('properties:editProperty')}
                      </button>
                    </div>
                  </div>
          ...
          {!loading && !error && filteredProperties.length === 0 && (
            <div className="py-20 flex flex-col items-center justify-center text-center bg-white rounded-[24px] border border-dashed border-gray-200 shadow-sm">
              ...
              <h3 className="text-xl font-serif font-bold text-gray-800 mb-2">{t('properties:noPropertiesFound')}</h3>
