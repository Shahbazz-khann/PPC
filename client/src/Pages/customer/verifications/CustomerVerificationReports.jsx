...
const VerificationCard = ({ report }) => {
  ...
  const locationString = [report.societyName, report.cityName].filter(Boolean).join(', ') || 'No location';
  const titleString = [report.propertyType, report.societyName].filter(Boolean).join(' in ') || 'Unknown Property';
  ...
  return (
    <div className="bg-white rounded-[24px] shadow-sm border border-gray-100 overflow-hidden flex flex-col md:flex-row hover:shadow-md transition-shadow min-h-[180px]">
      ...
      <div className="relative w-full md:w-[35%] xl:w-[28%] shrink-0 h-56 md:h-auto bg-gray-100 flex flex-col items-center justify-center">
        {imageSrc ? (
          <img src={imageSrc} alt="Property" className="w-full h-full object-cover" onError={(e) => { e.target.onerror = null; e.target.style.display = 'none'; e.target.parentElement.innerHTML = '<span class="text-gray-400 font-medium z-10 relative">No Image</span>' }} />
        ) : (
        ...
        <div className="p-5 lg:p-6 flex-1 flex flex-col justify-center border-b xl:border-b-0 xl:border-r border-gray-100 min-w-0">
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm font-semibold text-gray-500">
              ...
              <span>{report.propertyType || 'Unknown'}</span>
              {report.propertySize && (
                <>
                  <span className="w-1.5 h-1.5 rounded-full bg-gray-300"></span>
              ...
              <>
                ...
                <div>
                  ...
                  <span className="text-sm font-bold text-[#1a2b25] flex items-center gap-2">
                    <User size={14} className="text-[#1E5631]" /> {report.verifiedBy ? `${report.verifiedBy.name} - ${report.verifiedBy.designation}` : 'Not assigned yet'}
                  </span>
                </div>
              </>
            ) : (
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      if (res?.success) {
        ...
        setError(res?.message || 'Failed to load verification reports.');
      }
    } catch (err) {
      ...
      <div className=" px-4 sm:px-8 lg:px-12 xl:px-4 ">
        ...
        {loading ? (
          <div className="bg-white rounded-[24px] shadow-sm border border-gray-100 p-16 flex flex-col items-center justify-center">
             ...
             <p className="text-gray-500 font-medium text-sm">Loading verification reports...</p>
          </div>
        ) : error ? (
